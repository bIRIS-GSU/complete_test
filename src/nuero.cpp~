...
void EvReturnInfo(const complete_test::snnevent::ConstPtr& msg){

	
 int EvRenable = msg->enable;

 double ach, dopa, sero, openfield, explore, wallfollow, findhome;
	
	global_ach = msg->ach;
	global_dopa = msg->dopa;
	global_sero = msg->sero;
	global_openfield = msg->openfield;
	global_explore = msg->explore;
	global_wallfollow = msg->wallfollow;
	global_findhome = msg->findhome;

	//global_ach = ach;
	//global_dopa = dopa;
	//global_sero = sero;
	//global_openfield = openfield;
	//global_explore = explore;
	//global_wallfollow = wallfollow;
	//global_findhome = findhome;

	int snn_action = 0;
	float c;
	int I;
	vector <double> snn_vector(4,1);
	snn_vector[0] = wallfollow;
	snn_vector[1] = openfield;
	snn_vector[2] = explore;
	snn_vector[3] = findhome;

	c = snn_vector[0]; //in case if all elements equal
	I = 0;	
	for (int i = 0;i < (snn_vector.size()-1);i++) 
	{
		if (c < snn_vector[i+1]){
			c = snn_vector[i+1];
			I = i+1;}
	...
	snn_action = I;
	ROS_INFO("SNN ACTION: [%i]", snn_action);
	ROS_INFO("SNN ACTION SPIKES: [%f]", c);

	snn_behave_state = snn_action;
	snn_enable = EvRenable;
}
...
void main_neuromodulated_program_direct_sensor()
{
	...
	chatter_pub3.publish(notice);

	while(snn_enable == 0){ //waits on SNN data
...
//ORIGINAL NEURAL CODE COMMENT OUT
/*
...
*/
 ...
 snn_behave_state = behave_state;	
    ...
    snnlogvec[0] = current_time;
    snnlogvec[1] = behave_state;
    //log(loginx, 3:3+e-1) = event;
    	snnlogvec[2] = event[0];
    	snnlogvec[3] = event[1];
    	snnlogvec[4] = event[2];
    	snnlogvec[5] = event[3];
   	 snnlogvec[6] = global_ach;
    	snnlogvec[7] = global_dopa;
  	snnlogvec[8]= global_sero;
	snnlogvec[9]= global_openfield;
	snnlogvec[10]= global_explore;
	snnlogvec[11]= global_wallfollow;
	snnlogvec[12]= global_findhome;
    snnloginx.push_back(snnlogvec);
 ...
 ofstream f("./src/complete_test/src/results.xls");
	...
	ofstream g("./src/complete_test/src/results_snn.xls");
	g<<"current_time";
	g<<" ";
	g<<"behave_state";
	g<<" ";
	...
	//writes data
	for (int i = 0; i < snnloginx.size(); i++)
  	{
  		for (int j = 0; j < snnloginx[i].size(); j++)
    	{
    		g << snnloginx[i][j];
    		g << " ";
    	}	
...
int main(int argc, char **argv)
{
	...
	ros::Subscriber sub5 = node_handle.subscribe("snn_event_return_4", 0, EvReturnInfo);
	...
	chatter_pub3 = node_handle.advertise<complete_test::snnevent>("snn_event_4", 1);
